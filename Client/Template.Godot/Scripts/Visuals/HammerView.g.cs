...
public partial class HammerView : Node3D
{
    [Export] public PackedScene Prefab;

    private readonly CompositeDisposable _disposables = new();
    private readonly Dictionary<HammerViewModel, Node3D> _spawnedEntities = new();
    ...
    private void Register()
    {
        ...
        var query = client.Reactive.ObservableCollection<HammerComponent, DTransform2D>();
        Func<Entity, bool> filter = null;
        GetEntityFilter(ref filter);
        if (filter != null) query = query.Where(filter);
        var list = query.ToObservableList(
            ctx => new HammerViewModel(ctx),
            _disposables
        );
    ...
    private void OnAdded(HammerViewModel vm)
    {
    ...
    private void OnRemoved(HammerViewModel vm)
    {
    ...
    {
        foreach (var vm in new List<HammerViewModel>(_spawnedEntities.Keys))
            OnRemoved(vm);
    }
