...
public partial class SmithyView : Node3D
{
    [Export] public PackedScene Prefab;

    private readonly CompositeDisposable _disposables = new();
    private readonly Dictionary<SmithyViewModel, Node3D> _spawnedEntities = new();
    ...
    private void Register()
    {
        ...
        var query = client.Reactive.ObservableCollection<SmithyComponent, DTransform2D>();
        Func<Entity, bool> filter = null;
        GetEntityFilter(ref filter);
        if (filter != null) query = query.Where(filter);
        var list = query.ToObservableList(
            ctx => new SmithyViewModel(ctx),
            _disposables
        );
    ...
    private void OnAdded(SmithyViewModel vm)
    {
    ...
    private void OnRemoved(SmithyViewModel vm)
    {
    ...
    {
        foreach (var vm in new List<SmithyViewModel>(_spawnedEntities.Keys))
            OnRemoved(vm);
    }
