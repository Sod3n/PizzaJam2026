...
public partial class LandSignView : Node3D
{
    [Export] public PackedScene Prefab;

    private readonly CompositeDisposable _disposables = new();
    private readonly Dictionary<LandSignViewModel, Node3D> _spawnedEntities = new();
    ...
    private void Register()
    {
        ...
        var list = client.Reactive.ObservableList<LandSignComponent, DTransform2D, LandSignViewModel>(
            ctx => new LandSignViewModel(ctx),
            _disposables
        );
    ...
    private void OnAdded(LandSignViewModel vm)
    {
    ...
    private void OnRemoved(LandSignViewModel vm)
    {
    ...
    {
        foreach (var vm in new List<LandSignViewModel>(_spawnedEntities.Keys))
            OnRemoved(vm);
    }
