...
public partial class RoleSignView : Node3D
{
    [Export] public PackedScene Prefab;

    private readonly CompositeDisposable _disposables = new();
    private readonly Dictionary<RoleSignViewModel, Node3D> _spawnedEntities = new();
    ...
    private void Register()
    {
        ...
        var list = client.Reactive.ObservableList<RoleSignComponent, DTransform2D, RoleSignViewModel>(
            ctx => new RoleSignViewModel(ctx),
            _disposables
        );
    ...
    private void OnAdded(RoleSignViewModel vm)
    {
    ...
    private void OnRemoved(RoleSignViewModel vm)
    {
    ...
    {
        foreach (var vm in new List<RoleSignViewModel>(_spawnedEntities.Keys))
            OnRemoved(vm);
    }
