...
public class LandPriceSignSystem : ISystem
{
    public void Update(EntityWorld state)
    {
        foreach (var playerEntity in state.Filter<InteractRequestComponent>())
        {
            ...
            if (!state.TryGetComponent<LandPriceSignComponent>(signEntity, out var priceSign)) continue;

            var landId = priceSign.LandId;
            if (!state.HasComponent<LandComponent>(landId)) continue;
    ...
    private static void DepositOnLand(EntityWorld state, Entity playerEntity, Entity signEntity, Entity landEntity)
    {
        ...
        if (landComplete)
        {
            ...
            if (state.TryGetComponent<CooldownComponent>(landEntity, out var cd))
                carry = cd;
