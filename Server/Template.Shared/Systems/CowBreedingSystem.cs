...
public class CowBreedingSystem : ISystem
{
        ...
        Entity loveHouseEntity, int breedCount, bool guaranteedUpgrade)
    {
        ...
        var babyHelper = Definitions.HelperDefinition.Create(ctx, spawnPos, neededHelper, playerEntity);
        {
    ...
    {
        if (cow1 != Entity.Null || cow2 != Entity.Null)
        {
            ref var lh = ref state.GetComponent<LoveHouseComponent>(loveHouseEntity);
            if (lh.CowId1 == cow1) lh.CowId1 = Entity.Null;
            if (lh.CowId2 == cow1) lh.CowId2 = Entity.Null;
            if (lh.CowId1 == cow2) lh.CowId1 = Entity.Null;
            if (lh.CowId2 == cow2) lh.CowId2 = Entity.Null;
        }
        ...
        int breedCount = 0, SkinComponent? twinSkin = null)
    {
        ...
        var spawnPos = state.HasComponent<Transform2D>(parentB)
            ? state.GetComponent<Transform2D>(parentB).Position + new Vector2(2, 0)
            : new Vector2(0, 0);
